...
export const formatInputLive = (text) => {
  ...
  // Разбиваем по блокам кода (inline `...` или fenced ```...```), чтобы не портить синтаксис
  ...
  for (let i = 0; i < parts.length; i += 2) {
    ...
    // 3. Кавычки-ёлочки («...») с учётом открывающейся/закрывающейся
    s = s.replace(/(^|[\s([{-])"/g, '$1«')
    s = s.replace(/"/g, '»')

    // 4. Неразрывный пробел после коротких слов/предлогов
...
// -------------------------------------------------- инспектор переводов (#298)

export const findTranslationKey = (text, dicts = {}, overrides = {}, zhRu = {}) => {
  if (!text || typeof text !== 'string') return null
