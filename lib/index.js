...
export function apply(ctx) {
  ...
  // One-click plugin updater per DSH standard
  if (typeof ctx.inject === 'function') {
    ctx.inject(['webServer'], (wctx) => {
      try {
        ...
        if (typeof wctx.effect === 'function') wctx.effect(mount, 'russian-lang: plugin updater')
        else mount()
      } catch (err) {
        if (typeof wctx.logger?.warn === 'function') {
          wctx.logger.warn('Failed to mount plugin updater route:', err)
        }
      }
    })
  }
