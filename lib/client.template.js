...
//__PURE_JS__
    ...
    function apply(ctx) {
      ...
      const syncLang = () => {
        try {
          if (typeof document !== 'undefined' && document.documentElement && runtime.getLocale().active === 'ru') document.documentElement.lang = 'ru-RU'
        } catch (err) { /* ignore */ }
      }
      ...
      ctx.effect(() => {
        const origTranslate = runtime.translate
        const origProps = {}
            ...
            if (typeof n === 'number') {
              ...
              if (m) {
                if (form === 'few' || form === 'many') {
                  const tmpl = lookup(ns, m[1] + '.' + form) ?? lookup('common', m[1] + '.' + form)
                  if (tmpl !== undefined) return fill(tmpl, params)
                }
              } else if (form !== 'other' && !/[.](one|other|few|many)$/.test(key)) {
                const tmpl = lookup(ns, key + '.' + form) ?? lookup('common', key + '.' + form)
                if (tmpl !== undefined) return fill(tmpl, params)
              }
            }
          }
        ...
        let removeLang = null
        if (!runtime.getLocale().locales.some((l) => l.id === 'ru') && typeof runtime.addLanguage === 'function') {
          removeLang = runtime.addLanguage({ id: 'ru', label: 'Русский', fallback: 'en' })
          syncLang()
        }
        return () => {
          runtime.translate = origTranslate
          for (const p of RUNTIME_PROP_NAMES) {
            if (origProps[p] !== undefined) runtime[p] = origProps[p]
            else delete runtime[p]
          }
          if (typeof removeLang === 'function') { try { removeLang() } catch (_) { /* bestEffort */ } }
        }
      }, 'dsh-russian-lang: runtime-lifecycle')
...
'Loading inventory & status...': 'Загрузка списка и статуса...'
    ...
    function SettingsCard(props) {
      ...
      const startLibreTranslate = () => {
        ...
        setTransMsg(null)
        fetch('/api/dsh-russian-lang/translator/setup', {
          method: 'POST',
