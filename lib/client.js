...
const formatDate = (val, style = 'short') => {
...
const formatTokens = (count, compact = false) => {
...
function plural(count, forms, withCount = false) {
...
const SYSTEM_PROMPT_PRESETS = {
  ...
  code_reviewer: {
    id: 'code_reviewer',
    label: 'Код-ревьюер',
    desc: 'Поиск багов, безопасность, архитектурный аудит и исправления',
    text: 'Проводи глубокий аудит кода и отвечай пользователю на русском языке. Ищи критические уязвимости, утечки памяти, ошибки асинхронности, граничные случаи и нарушения контрактов. Формулируй замечания конструктивно, предлагая конкретный исправленный код.'
  },
  architect: {
    id: 'architect',
    label: 'Системный архитектор',
    desc: 'Контракты API/CLI, модульность, масштабируемость и надёжность',
    text: 'Действуй как системный архитектор и отвечай пользователю на русском языке. Анализируй требования, предлагай модульную архитектуру, чёткие контракты интерфейсов (API/CLI) и решения, устойчивые к отказам. Избегай избыточных абстракций (YAGNI/KISS), объясняй компромиссы и решения на русском языке.'
  },
  tutor: {
    id: 'tutor',
    label: 'Наставник (ментор)',
    desc: 'Понятные объяснения сложных концепций и пошаговый разбор',
    text: 'Отвечай пользователю на русском языке как опытный наставник. Объясняй сложные концепции простым и понятным языком с наглядными аналогиями и пошаговыми примерами. Поощряй понимание принципов, а не механическое копирование решений.'
  }
}
...
const findTranslationKey = (text, dicts = {}, overrides = {}, zhRu = {}) => {
  if (!text || typeof text !== 'string') return null
                ...
                React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' } },
                  React.createElement('div', null,
                    React.createElement('div', { style: { fontWeight: 600, color: 'var(--dsw-alias-label-primary)' } },
                      t('updaterCurrent').replace('{version}', upStatus.currentVersion || '0.3.0')),
