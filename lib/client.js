...
const findTranslationKey = (text, dicts = {}, overrides = {}, zhRu = {}, domEn = {}, options = {}) => {
if (!text || typeof text !== 'string') return null
...
const generateBugReportSnippet = (info = {}) => {
const { key, ns, original, current, override, pkgVersion = '0.3.1' } = info
const lines = [
'### 🌐 Неточность перевода / Пользовательское предложение',
'- **Пакет:** `@goodandready/dsh-russian-lang@v' + pkgVersion + '`',
...
const bilingualCommandMatch = (query, command, options = {}) => {
...
const filterCommands = (query, commands = [], options = {}) => {
...
if (copyKeyBtn) {
copyKeyBtn.onclick = () => {
try { navigator.clipboard.writeText(k || rawText) } catch (err) {  void err; }
...
? generateBugReportSnippet({ key: k, ns, original: origText || rawText, current: rawText, override: textarea.value.trim(), pkgVersion: '0.3.2' })
...
try { setLocaleState(runtime.getLocale().active || 'en') } catch (e) {  }
})
}, [runtime])
...
setDone(true)
setTimeout(() => {
try { document.body.removeChild(a); URL.revokeObjectURL(url) } catch (e) {  void e; }
}, 30000)
...
e.stopPropagation()
try { navigator.clipboard.writeText(displayText) } catch (err) {  void e; }
...
try { setRuActive(runtime.getLocale().active === 'ru') } catch (e) {  }
})
return un
...
(ev) => {
try { scope.set('slashAliases', ev.target.checked) } catch (err) {  void e; }
}, !ruActive || disabled),
t('slashAliases')
)
),
...
t('updaterCurrent').replace('{version}', upStatus.currentVersion || '0.3.2')),
