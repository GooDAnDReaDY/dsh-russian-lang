...
const lines = [
'### 🌐 Неточность перевода / Пользовательское предложение','- **Пакет:** `@goodandready/dsh-russian-lang@v' + pkgVersion + '`',ns ? '- **Пространство имён:** `' + ns + '`' : null,key ? '- **Ключ:** `' + key + '`' : null,original ? '- **Оригинальный текст:** `' + original + '`' : null,current ? '- **Текущий перевод:** `' + current + '`' : null,override ? '- **Предлагаемый перевод:** `' + override + '`' : null,].filter(Boolean)
return lines.join('\n')
}
...
}).catch(() => {if (typeof fetch === 'function') {fetch('/api/dsh-russian-lang/dict/all', { headers: { 'Accept': 'application/json' } })
...
? generateBugReportSnippet({ key: k, ns, original: origText || rawText, current: rawText, override: textarea.value.trim(), pkgVersion: '0.3.8' })
...
try {const res = await fetch('/api/dsh-russian-lang/translate', {headers: { 'content-type': 'application/json', 'x-dsh-translator': '1' },method: 'POST',headers: { 'content-type': 'application/json' },body: JSON.stringify({ text })
})
...
const [upStatus, setUpStatus] = React.useState({currentVersion: '0.3.8',latestVersion: undefined,updateAvailable: false
})
const [upLoading, setUpLoading] = React.useState(false)
const [upMsg, setUpMsg] = React.useState(null)
const checkUpdate = () => {setUpLoading(true)
setUpMsg(null)
fetch('/api/dsh-russian-lang/update', {headers: { 'x-dsh-plugin-update': '1' }
})
.then((r) => r.json())
...
setUpMsg(null)
fetch('/api/dsh-russian-lang/update', {method: 'POST',headers: { 'x-dsh-plugin-update': '1' }
})
.then((r) => r.json())
...
const fetchTransStatus = () => {fetch('/api/dsh-russian-lang/translator/status')
.then((r) => r.json())
...
setTransMsg(null)
fetch('/api/dsh-russian-lang/translator/setup', {method: 'POST',headers: { 'x-dsh-translator': '1' }
})
.then((r) => r.json())
...
),React.createElement('div', { className: 'rl-item-card' },React.createElement('div', { className: 'rl-item-head' },React.createElement('label', { className: 'rl-item-label' },checkbox(value.slashAliases !== false && ruActive,(ev) => {try { scope.set('slashAliases', ev.target.checked) } catch (err) {  void e; }
...
: React.createElement('div', { style: { color: 'var(--dsw-alias-label-secondary)', marginTop: '2px' } },t('updaterUpToDate'))
...
),React.createElement('div', { className: 'rl-actions-row' },React.createElement('a', {href: makeIssueUrl({}, '0.3.8'),target: '_blank',rel: 'noopener noreferrer',className: 'rl-btn rl-btn-primary'
